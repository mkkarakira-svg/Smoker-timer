...
class MainActivity: Activity(){
 lateinit var count:TextView; lateinit var monthCount:TextView; lateinit var timer:TextView; lateinit var last:TextView; lateinit var status:TextView
 ...
 root.addView(tv("SMOKER TIMER",14f));val monthKey="month_"+SimpleDateFormat("yyyyMM",Locale.US).format(Date());val monthTotal=prefs.getInt(monthKey,0)+n;monthCount=tv("This month: "+monthTotal+" cigarettes",14f);root.addView(monthCount);count=tv(n.toString()+" cigarettes today",22f);root.addView(count);timer=tv("READY",42f);timer.background=GradientDrawable().apply{setColor(Color.WHITE);cornerRadius=160f};root.addView(timer,LinearLayout.LayoutParams(360,190));last=tv(if(lastMs>0)"Last "+SimpleDateFormat("HH:mm",Locale.getDefault()).format(Date(lastMs)) else "Last —",14f);root.addView(last)
 root.addView(Button(this).apply{text="🚬  I SMOKED";textSize=18f;setOnClickListener{smoked()}},LinearLayout.LayoutParams(-1,120))
 root.addView(Button(this).apply{text="🕘 LOG EARLIER";setOnClickListener{val cal=Calendar.getInstance();TimePickerDialog(this@MainActivity,{_,hh,mm->val t=Calendar.getInstance().apply{set(Calendar.HOUR_OF_DAY,hh);set(Calendar.MINUTE,mm);set(Calendar.SECOND,0);set(Calendar.MILLISECOND,0)}.timeInMillis;if(t<=System.currentTimeMillis()){n++;lastMs=t;end=t+interval*60000L;prefs.edit().putInt("count",n).putLong("last",t).putLong("end",end).apply();count.text=n.toString()+" cigarettes today";monthCount.text="This month: "+(prefs.getInt("month_"+SimpleDateFormat("yyyyMM",Locale.US).format(Date()),0)+n)+" cigarettes";last.text="Last "+SimpleDateFormat("HH:mm",Locale.getDefault()).format(Date(t));if(end>System.currentTimeMillis())schedule();status.text="✓ Earlier cigarette logged"}else Toast.makeText(this@MainActivity,"Choose an earlier time",Toast.LENGTH_SHORT).show()},cal.get(Calendar.HOUR_OF_DAY),cal.get(Calendar.MINUTE),true).show()}},LinearLayout.LayoutParams(-1,90))
 root.addView(tv("INTERVAL",13f));val row=LinearLayout(this);listOf(30,45,60,90).forEach{minutes->row.addView(Button(this).apply{text=minutes.toString()+"m";setOnClickListener{interval=minutes;prefs.edit().putInt("interval",minutes).apply();if(lastMs>0){end=lastMs+minutes*60000L;prefs.edit().putLong("end",end).apply();if(end>System.currentTimeMillis())schedule()};status.text="Interval: "+minutes+" min"}},LinearLayout.LayoutParams(0,100,1f))};root.addView(row)
...
status=tv("Ready",12f);root.addView(status);setContentView(root);tick()}
 fun smoked(){n++;val now=System.currentTimeMillis();end=now+interval*60000L;prefs.edit().putInt("count",n).putLong("end",end).putLong("last",now).apply();count.text=n.toString()+" cigarettes today";monthCount.text="This month: "+(prefs.getInt("month_"+SimpleDateFormat("yyyyMM",Locale.US).format(Date()),0)+n)+" cigarettes";last.text="Last "+SimpleDateFormat("HH:mm",Locale.getDefault()).format(Date(now));schedule();status.text="✓ Logged"}
