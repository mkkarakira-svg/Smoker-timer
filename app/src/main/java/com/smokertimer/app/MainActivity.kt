...
class MainActivity: Activity(){
 ...
 override fun onCreate(b:Bundle?){super.onCreate(b);val today=SimpleDateFormat("yyyyMMdd",Locale.US).format(Date());if(prefs.getString("day","")!=today)prefs.edit().putInt("count",0).putString("day",today).apply();n=prefs.getInt("count",0);end=prefs.getLong("end",0);lastMs=prefs.getLong("last",0);interval=prefs.getInt("interval",60)
 ...
 root.addView(Button(this).apply{text="🔔 TEST ALARM — 1 MIN";setOnClickListener{end=System.currentTimeMillis()+60000L;prefs.edit().putLong("end",end).apply();schedule();status.text="Test alarm set for 1 minute"}},LinearLayout.LayoutParams(-1,90));status=tv("Ready",12f);root.addView(status);setContentView(root);tick()}
