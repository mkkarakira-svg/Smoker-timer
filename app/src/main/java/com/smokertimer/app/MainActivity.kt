...
class MainActivity: Activity(){
 ...
 override fun onCreate(b:Bundle?){super.onCreate(b);val today=SimpleDateFormat("yyyyMMdd",Locale.US).format(Date());if(prefs.getString("day","")!=today){val old=prefs.getInt("count",0);val month=SimpleDateFormat("yyyyMM",Locale.US).format(Date());prefs.edit().putInt("month_"+month,prefs.getInt("month_"+month,0)+old).putInt("count",0).putString("day",today).putLong("end",0).putLong("last",0).apply()}\nn=prefs.getInt("count",0);end=prefs.getLong("end",0);lastMs=prefs.getLong("last",0);interval=prefs.getInt("interval",60)
